...
- 게시글(분실물 카드)
  ...
  - 상태 필터: `GET /api/articles?status=lost|found|resolved`.
...
- 프론트 데모 (`src/main/resources/static/index.html`)
  - 상태 셀렉트/필터/배지, 내 글 탭, 해결 버튼, 답글/삭제, 커서 더보기.
...
- 게시글:
  - `GET /api/articles?lastId&size&status` 커서+상태 필터 목록
...
- 배포 시 URL/계정은 상단에 추가, DB/시크릿은 환경변수로 관리.***
